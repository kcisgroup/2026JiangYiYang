...
DATA_PATH = os.path.join(BASE_DIR, 'MyModel/dataset_process/foursquare_tky_cleaned.csv')  # 清洗后的签到数据路径
DISTANCE_DATA_PATH = os.path.join(BASE_DIR, 'MyModel/dataset_process/distance_df_tky.csv')  # 预计算的POI间距离数据路径
MODEL_SAVE_DIR = os.path.join(BASE_DIR, 'MyModel/GSCAT/TKY/OptunaRuns')  # Optuna运行和模型保存目录
...
PRETRAINED_EMBED_DIR = os.path.join(BASE_DIR, 'MyModel/GSCAT/TKY/PretrainedEmbeddings')  # 预训练嵌入保存目录
...
DEVICE = torch.device(f"cuda:2" if torch.cuda.is_available() else "cpu")  # 设置计算设备
SEED = 42  # 随机种子，保证可复现性
...
def main():
    ...
    print("\n" + "="*30 + " 保存 Stage 2 所需的全量特征 " + "="*30)
    
    STAGE2_DIR = os.path.join(BASE_DIR, 'MyModel/Stage2_Features/TKY')
    os.makedirs(STAGE2_DIR, exist_ok=True)

    # 1. 保存 ID 映射表
    mapping_save_path = os.path.join(STAGE2_DIR, "id_mappings.pkl")
    ...
    print(f"[*] ID 映射已保存至: {mapping_save_path}")

    final_model.eval()
    
    # 2. 提取并保存所有的 POI 特征 (全局空间多关系图)
    with torch.no_grad(), autocast('cuda', enabled=True):
        global_poi_reps = final_model(
            batch=None, 
            mode='compute_global_reps', 
            global_graph_data=global_poi_graph_data, 
            config=best_config
        )
        if global_poi_reps is not None:
            poi_reps_save_path = os.path.join(STAGE2_DIR, "pretrained_poi_embs.pt")
            torch.save(global_poi_reps.cpu().detach(), poi_reps_save_path)
            print(f"[*] 全局 POI 拓扑特征已保存，维度: {global_poi_reps.shape}")

    # 3. 提取并保存全量用户的画像特征
    print(">>> 正在使用训好的 GSCAT 提取全城所有用户的偏好画像...")
    
    # 使用包含【全量用户】的 df_proc 创建 Dataset
    full_user_ds = UserFullHistoryDataset(
        df_proc.groupby('user_id'),
        venue_map, category_map, MAX_SEQ_LENGTH,
        venue_pad_idx, cat_pad_idx,
        EDGE_TIME_DIFF_BINS_GAT, EDGE_DIST_BINS_GAT, global_distance_lookup,
        is_train=False, edge_dropout_rate=0.0
    )
    
    collate_fn_user_rep = partial(user_full_history_gat_collate_fn, 
                                  venue_pad_idx_g=venue_pad_idx, 
                                  cat_pad_idx_g=cat_pad_idx, 
                                  time_segment_pad_idx_g=TIME_SEGMENT_PAD_IDX)
                                  
    full_user_dl = DataLoader(full_user_ds, batch_size=BATCH_SIZE, shuffle=False, 
                              collate_fn=collate_fn_user_rep, num_workers=4)

    # 准备一个 [1083, User_Dim] 的空张量
    user_dim = final_model.user_rep_module.user_rep_final_dim
    all_user_reps_tensor = torch.zeros(len(user_map), user_dim)
    
    with torch.no_grad(), autocast('cuda', enabled=True):
        for batch_data in tqdm(full_user_dl, desc="生成全量用户特征"):
            if batch_data is None: continue
            
            # 将原始 user_id (如字符串或原ID) 转换为我们字典里的连续 index
            raw_user_ids = batch_data['user_ids']
            u_indices = [user_map[uid] if uid in user_map else uid for uid in raw_user_ids]
            
            # 放到 GPU
            for key in batch_data:
                if isinstance(batch_data[key], (torch.Tensor, Data)):
                    batch_data[key] = batch_data[key].to(DEVICE)
            
            # 使用 Stage 1 模型计算这段历史轨迹的最终特征
            reps = final_model.user_rep_module(batch_data, global_poi_representations=global_poi_reps)
            
            # 写回大张量
            for i, u_idx in enumerate(u_indices):
                if isinstance(u_idx, int) and 0 <= u_idx < len(user_map):
                    all_user_reps_tensor[u_idx] = reps[i].cpu().detach()
                    
    user_reps_save_path = os.path.join(STAGE2_DIR, "pretrained_user_embs.pt")
    torch.save(all_user_reps_tensor, user_reps_save_path)
    print(f"[*] 全量用户画像特征已保存！维度: {all_user_reps_tensor.shape}")
