...
class GlobalPOIGraphBuilder:
    ...
    def build_global_graph(self):
        ...
        if not all_edges:
            ...
            return Data(x=node_features, edge_index=torch.empty(2, 0, dtype=torch.long), num_nodes=num_nodes_from_features) # type: ignore
        ...
        # 创建最终的图数据对象
        graph_data = Data(x=node_features, edge_index=edge_index, edge_attr=edge_attr, num_nodes=num_nodes_from_features) # type: ignore
...
class UserFullHistoryDataset(Dataset):
    ...
    """
    def __init__(self, df_user_histories_grouped: pd.core.groupby.generic.DataFrameGroupBy, # type: ignore
...
def user_full_history_gat_collate_fn(batch, venue_pad_idx_g, cat_pad_idx_g, time_segment_pad_idx_g):
    ...
    for item in batch:
        ...
        default_empty_gat_data = Data( # type: ignore
    ...
    if gat_data_list_for_pyg_batch:
        try:
            stacked_data['gat_batch'] = Batch.from_data_list(gat_data_list_for_pyg_batch) # type: ignore
        except Exception as e:
            print(f"Error in Batch.from_data_list: {e}")
            stacked_data['gat_batch'] = Batch() # type: ignore
    else:
        stacked_data['gat_batch'] = Batch() # type: ignore
...
class MultiRelationalPOIEncoder(nn.Module):
    ...
    def forward(self, global_graph_data, target_node_indices=None):
        ...
        if self.treat_as_single_relation:
            ...
            for gat_layer in gat_stack: # type: ignore
                # 使用图中所有的边
...
class SequenceEncoder(nn.Module):
    ...
    def __init__(self, d_model, nhead, num_encoder_layers, dim_feedforward, dropout,
        ...
        if self.use_global_graph:
            ...
            self.global_graph_proj = nn.Linear(global_poi_encoder.hidden_dim, d_model) # type: ignore
    ...
    def forward(self, user_history_batch_dict, global_poi_representations=None):
        ...
        if self.use_gat and 'gat_batch' in user_history_batch_dict:
            ...
            if gat_batch_data is not None and gat_batch_data.num_graphs > 0 and hasattr(gat_batch_data, 'x_node_venue_ids') and gat_batch_data.x_node_venue_ids.numel() > 0:
                ...
                graph_level_rep_batched = global_mean_pool(projected_gat_node_h, gat_batch_data.batch) # type: ignore
...
class TrajectoryTransformer(nn.Module):
    ...
    def init_non_encoder_weights(self):
        ...
        if self.attention_pooling and hasattr(self.attention_pooling, 'attention_net'):
            modules_to_init.append(self.attention_pooling.attention_net) # type: ignore
            
        if hasattr(self, 'fusion_with_similar_users_mlp') and self.fusion_with_similar_users_mlp is not None:
            modules_to_init.append(self.fusion_with_similar_users_mlp) # type: ignore
        ...
        if hasattr(self, 'similar_user_gate_mlp') and self.similar_user_gate_mlp is not None:
            modules_to_init.append(self.similar_user_gate_mlp) # type: ignore
    ...
    def forward(self, venues, hours, time_segment_types, cats, lats, lons, popularities,
        ...
        if self.use_similar_user_fusion:
            ...
            if conditions_met:
                ...
                all_train_user_reps_tensor = all_train_user_reps_tensor.to(current_device) # type: ignore

                for i in range(B):
                    # a. 找到当前用户的预计算画像
                    current_user_id = current_user_ids_batch[i] # type: ignore # type: ignore
                    ...
                    current_user_idx = train_user_id_to_idx_map[current_user_id] # type: ignore
                ...
                # f. 使用门控机制调整融合强度
                if self.similar_user_fusion_gate_type == 'simple_sigmoid':
                    gate = torch.sigmoid(self.similar_user_gate_fc(pooled_output)) # type: ignore
                    fused_info = fused_similar_user_info * gate
                elif self.similar_user_fusion_gate_type == 'contextual_mlp':
                    gate_mlp_in = torch.cat([pooled_output, aggregated_similar_reps_tensor], dim=1)
                    logits2 = self.similar_user_gate_mlp(gate_mlp_in) # type: ignore
                    ...
                    gate = torch.sigmoid(self.final_concat_gate_fc(temp_concat.detach())) # type: ignore
                    fused_info = fused_similar_user_info * gate
                else:
            ...
            if conditions_met and self.similar_user_fusion_gate_type == 'contextual_mlp':
                if 'logits2' in locals():
                    g1 = torch.sigmoid(logits2[:, 0:1]) # type: ignore
    ...
    with torch.no_grad():
        ...
        for batch_data in pbar:
            ...
            for key_b in batch_data:
                ...
                elif isinstance(batch_data[key_b], Data): # type: ignore # 处理PyG的Data或Batch对象
                     batch_data[key_b] = batch_data[key_b].to(device)
            ...
            if is_cache_system:
                ...
                try:
                    ...
                    graph_data = global_graph_data.get_poi_graph_for_batch(batch_data, "user_rep_update") # type: ignore
...
class AblationStudyRunner:
    ...
    def _prepare_data(self):
        """一次性加载和准备所有数据，包括TransE嵌入"""
        print("\n" + "="*80)
        print("Step 1: 准备数据、用户划分、构建全局图及加载嵌入")
        print("="*80)
        ...
        # 3. 加载预训练嵌入 (TransE)
        # 根据数据集名称推断路径
        dataset_code = "NYC" if "NYC" in self.dataset_name else "TKY"
        base_dir = os.getcwd()
        embed_dir = os.path.join(base_dir, f'MyModel/GSCAT/{dataset_code}/PretrainedEmbeddings')
        poi_emb_path = os.path.join(embed_dir, 'poi_embeddings_transe.pt')
        cat_emb_path = os.path.join(embed_dir, 'cat_embeddings_transe.pt')
        
        if os.path.exists(poi_emb_path) and os.path.exists(cat_emb_path):
            print(f"检测到预训练嵌入文件，正在加载...\n  POI: {poi_emb_path}\n  CAT: {cat_emb_path}")
            try:
                self.poi_embeds = torch.load(poi_emb_path, map_location=DEVICE)
                self.cat_embeds = torch.load(cat_emb_path, map_location=DEVICE)
                print("TransE 嵌入加载成功。")
            except Exception as e:
                print(f"加载嵌入时出错: {e}。将不使用预训练嵌入。")
                self.poi_embeds = None
                self.cat_embeds = None
        else:
            print(f"未找到预训练嵌入文件 (期望路径: {embed_dir})。将不使用预训练嵌入。")
    ...
    def _run_single_experiment(self, config):
        ...
        if exp_name == "GSCAT (Full Model)":
            ...
            run_config['dropout_shared'] = min(0.9, run_config.get('dropout_shared', 0.35) + 0.1)
            run_config['global_graph_dropout'] = min(0.9, run_config.get('global_graph_dropout', 0.2) + 0.1)
            run_config['gat_dropout'] = min(0.9, run_config.get('gat_dropout', 0.23) + 0.1)
        ...
        # --- 核心修改：嵌入处理逻辑 ---
        current_poi_embeds = self.poi_embeds
        current_cat_embeds = self.cat_embeds
        
        # 如果配置明确要求禁用 KG Pretraining，则设为 None
        if run_config.get("disable_kg_pretrain", False):
            print("  [Ablation] 检测到 disable_kg_pretrain=True。显式禁用预训练嵌入。")
            current_poi_embeds = None
            current_cat_embeds = None
        else:
            if current_poi_embeds is not None:
                print("  [Config] 使用加载的 TransE 预训练嵌入。")
            else:
                print("  [Config] 这是一个需要嵌入的实验，但内存中没有嵌入 (随机初始化)。")

        # 实例化模型
        model = POIRecommender(
            config=config, **model_fixed_params, 
            poi_embeds_pt=current_poi_embeds, 
            cat_embeds_pt=current_cat_embeds
        ).to(DEVICE)
        ...
        if needs_user_rep_dl and df_train_for_user_rep is not None:
            user_history_ds = UserFullHistoryDataset(
                ...
                EDGE_TIME_DIFF_BINS_GAT, EDGE_DIST_BINS_GAT, self.global_distance_lookup, # type: ignore
        ...
        for ep in range(1, epochs + 1):
            train_metrics_loss_only = train_epoch(
                model, optimizer, scheduler, self.dataloaders['train'], user_rep_dl, # type: ignore
    ...
    def _define_ablation_configs(self):
        ...
        configs = {
            ...
            # --- 新增消融项 ---
            "GSCAT w/o KG-Pretrain": {**self.base_best_config, "disable_kg_pretrain": True},
            # ----------------
